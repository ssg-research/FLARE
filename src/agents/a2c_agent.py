...
class a2c_agent(base_agent):
    def __init__(self, name, agent_mode, env, device, args):
        base_agent.__init__(self, name, agent_mode, env, device, args)
        # define the network
    ...
    def select_action(self, inputs, rnn_hxs, masks, log_probs=False, deterministic=True):
        ...
        if log_probs:
            ...
            return value, action.squeeze(0), action_log_probs, complete_action_log_probs.squeeze(0)
        else:
            return value, action.squeeze(0), action_log_probs, rnn_hxs

    def get_value(self, inputs, rnn_hxs, masks):
        value, _, _ = self.net(inputs, rnn_hxs, masks)
        return value

    def update_agent(self, j, rollouts):
